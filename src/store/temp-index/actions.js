...
export async function fetchData ({ commit }, id) {
  try {
    ...
    pt = await pt
    vid = await vid
    commit('setTag', { win: win.data[0].tag_name, ios: ios.data[0].tag_name, mac: mac.data[0].tag_name })
    commit('setPublish', { win: win.data[0].published_at, ios: ios.data[0].published_at, mac: mac.data[0].published_at })
